      ...
      <div className="relative flex overflow-hidden">
        <div className="flex items-center gap-16 md:gap-24 animate-marquee whitespace-nowrap">
          {reel.map((p, i) => (
            <div
              key={`${p.name}-${i}`}
              className="relative flex items-center justify-center h-16 md:h-20 w-36 md:w-48 shrink-0 rounded-xl bg-gradient-to-b from-white/[0.06] to-white/[0.02] border border-white/10 backdrop-blur-md px-4 py-3 transition-all hover:border-primary/40 hover:from-primary/10 hover:to-primary/5"
            >
              {p.logoUrl ? (
                <Image
                  src={p.logoUrl}
